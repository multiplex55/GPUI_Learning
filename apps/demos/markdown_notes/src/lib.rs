...
pub fn run() {
    ...
    app.run(|cx| {
        ...
        launch(cx, registry.clone(), store.clone(), config.clone());

        cx.activate(true);
    });
}

pub fn launch(cx: &mut App, registry: ThemeRegistry, store: ConfigStore, config: WorkspaceConfig) {
    cx.open_window(
        WindowOptions {
